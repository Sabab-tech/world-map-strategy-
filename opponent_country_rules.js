...
const DATASETS={countries:['countries.json',1],economy:['economy.json',0],population:['population.json',0],resources:['resources.json',0],cities:['cities.json',0],world:['world.json',0],industry:['industry.json',0],infrastructure:['infrastructure.json',0],finance:['finance.json',0],projects:['projects.json',0],relations:['relations.json',0],events:['events.json',0]};
...
function WORLD(){return g.Game?.state||g.gameState||g.Omega?.AuthoritativeWorldState?.state||{};} function SIM(){return g.Omega?.Simulation||g.Omega?.SimulationRuntime||null;} function IO(){return g.Omega?.MinistryInteroperability||g.OmegaMinistryInteroperability||null;}
...
class Kernel{constructor(gw,id,tr){this.gw=gw;this.id=id;this.tr=tr;this.hyd=new Map;}snap(c,t){const st=WORLD(),sig={};for(const k of Object.keys(DIR)){let v,src=null;for(const p of STATE_PATHS[k]||[]){v=STATE_PATH(st,c,p);if(v!==undefined){src='STATE:'+p;break;}}if(v===undefined)for(const[d,raw]of this.gw.c){const r=REC(raw,c);if(!r)continue;for(const p of [k,...(FIELD_ALIASES[k]||[])]){const z=READ(r,p);if(z!==undefined){v=z;src='DATASET:'+d+'.'+p;break;}}if(v!==undefined)break;}sig[k]={signalId:k,value:v??null,raw:CLONE(v),direction:DIRECTION(v),status:v==null?'UNAVAILABLE':'AVAILABLE',source:src,provenance:src?{source:src,simulationTurn:t}:null};}const out={countryId:ID(c),turn:t,identity:this.id.resolve(c),signals:sig};this.hyd.set(ID(c),CLONE(out));this.tr.add({layer:'L03_WORLD_STATE_KERNEL',countryId:ID(c),available:Object.values(sig).filter(x=>x.status==='AVAILABLE').length,missing:Object.values(sig).filter(x=>x.status!=='AVAILABLE').length,stateHydrated:true});return out;}getHydrated(c){return CLONE(this.hyd.get(ID(c))||null);}}
...
function TRIGGER(t,c){if(!t)return{state:'UNKNOWN'};if(t.type==='signal'){const o=c.signals[t.signal];return !o||o.status!=='AVAILABLE'?{state:'UNKNOWN',reason:'SIGNAL_UNAVAILABLE'}:{...EVAL_DIR(t.signal,o.raw,t.direction),evidence:o};}if(t.type==='relation')return{...EVAL_REL(c,t.relation),evidence:{relation:t.relation}};if(t.type==='event'){const e=EVENT_LIST(c).find(x=>!x.resolved&&ID(x.eventType||x.type)===ID(t.event));return{state:e?'TRUE':'FALSE',evidence:e||{event:t.event}};}if(t.type==='all'){const r=t.items.map(x=>TRIGGER(x,c));if(r.some(x=>x.state==='UNKNOWN'))return{state:'UNKNOWN',evidence:r};return{state:r.every(x=>x.state==='TRUE')?'TRUE':'FALSE',evidence:r};}if(t.type==='any'){const r=t.items.map(x=>TRIGGER(x,c));if(r.some(x=>x.state==='TRUE'))return{state:'TRUE',evidence:r};if(r.every(x=>x.state==='FALSE'))return{state:'FALSE',evidence:r};return{state:'UNKNOWN',evidence:r};}return{state:'UNKNOWN',reason:'TRIGGER_UNSUPPORTED'};}
...
class Runtime{
  constructor(o={}){
    ...
    this.decision=new Decision(this.tr,this.feas);
