...
(function(global){
  ...
  function purgeLegacyLog(){
    ...
    host.replaceChildren();const head=document.createElement('div');head.dataset.omegaCanonicalMessage='1';head.textContent='--- CANONICAL INTERROGATION SESSION ---';head.style.cssText='font-size:11px;color:#64748b;font-family:var(--font-mono);text-align:center;padding:4px;';host.appendChild(head);host.dataset.omegaCanonicalPurged='true';
  }
  ...
  function patchInterrogationAPI(){
    ...
    global.__OMEGA_LEGACY_INTERROGATION__={originalAskCustom,originalAskPreset,originalProcess};purgeLegacyLog();syncVisibleIdentity();return true;
  }
  ...
  function boot(){
    installCapture();syncLayerIsolation();purgeLegacyLog();syncVisibleIdentity();loadIntegrityOnce();window.addEventListener('OMEGA_MINISTER_RUNTIME_READY',()=>syncVisibleIdentity(),false);
