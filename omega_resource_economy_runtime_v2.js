...
(function(g){
  ...
  function applyProductionHandler(cmd,ctx){
    ...
    var transportReady=g.OmegaResourceTransport&&typeof g.OmegaResourceTransport.inventoryAvailableForProcessing==='function';
    ...
    for(var i=0;i<inputs.length;i++){
      ...
      if(transportReady){
        available=g.OmegaResourceTransport.inventoryAvailableForProcessing(ctx.countryId,rid);
        if(available===null||available===undefined)available=key==null?0:num(inv[key]);
      }
  ...
  function dashboard(c){
    ...
    var refiningInput={},refiningOutput={};
    (Array.isArray(runtime.productionLedger)?runtime.productionLedger:[]).forEach(function(x){if(num(x.turn)!==turn())return;Object.keys(x.inputQuantities||{}).forEach(function(rid){refiningInput[rid]=(refiningInput[rid]||0)+(num(x.inputQuantities[rid])||0);});Object.keys(x.outputQuantities||{}).forEach(function(rid){refiningOutput[rid]=(refiningOutput[rid]||0)+(num(x.outputQuantities[rid])||0);});});
    var transportState=g.OmegaResourceTransport&&typeof g.OmegaResourceTransport.getCountryTransportState==='function'?g.OmegaResourceTransport.getCountryTransportState(cid):null;
    var shipments=g.OmegaResourceTransport&&typeof g.OmegaResourceTransport.getCountryShipments==='function'?g.OmegaResourceTransport.getCountryShipments(cid):[];
    return{version:VERSION,countryId:cid,turn:turn(),mines:{total:mines.length,active:active,blocked:blocked,rows:rows},inventory:{actual:clone(inv),delta:clone(rs.inventoryDelta||{}),batches:Array.isArray(rs.batches)?rs.batches.length:0,integrity:clone(rs.inventoryIntegrity||null)},industry:{runtime:clone(runtime),factoryOutput:clone(econ.factoryOutput||{}),blockedFacilities:clone(runtime.blockedFacilities||[]),refiningInput:refiningInput,refiningOutput:refiningOutput},transport:{runtime:clone(transportState||{}),shipments:clone(shipments),shipmentCount:Array.isArray(shipments)?shipments.length:0,inTransit:Array.isArray(shipments)?shipments.filter(function(x){return id(x.status)==='IN_TRANSIT';}).length:0,delivered:Array.isArray(shipments)?shipments.filter(function(x){return id(x.status)==='DELIVERED';}).length:0},market:{prices:clone(tr.marketPrice||{}),domesticSalesTotal:sales.length,domesticSalesThisTurn:sales.filter(function(x){return num(x.turn)===turn();}).length,autoOffers:offerRows.filter(function(x){return x&&x.source==='OMEGA_RESOURCE_ECON_AUTO_OFFER';}).length},treasury:{fiscalThisTurn:clone(currentFiscal),fiscalCumulative:clone(fisc.cumulative||{}),revenue:num(fin.revenue),taxRevenue:num(fin.taxRevenue),available:num(fin.available),reserves:num(fin.reserves),resourceBudgetContribution:num(fin.resourceBudgetContribution),resourceBudgetContributionThisTurn:fisc.lastTurn===turn()?(num(fin.resourceBudgetContributionThisTurn)||0):0,transportRevenue:num(tp.resourceRevenue&&tp.resourceRevenue.totalSinceRuntimeStart),companyAccounts:clone(econ.companyAccounts||{})},health:{status:blocked>0||runtime.status==='DEGRADED'||rs.inventoryIntegrity&&rs.inventoryIntegrity.status==='DEGRADED'?'DEGRADED':(mines.length?'HEALTHY':'NO_MINE_RECORDS'),worldMineCount:world,worldActiveMines:worldActive,worldBlockedMines:worldBlocked,runtimeVersion:VERSION,rulesVersion:rules().schemaVersion,noSyntheticWarehouseBalance:true},fiscalReceiptThisTurn:num(currentFiscal.total)||0};
  }
  ...
  function ui(){
    ...
    g.CountryIOS.renderChapter5_Resources=function(c){
      ...
      return'<div class="omega-re2">'+
        ...
        '<section class="omega-re2-flow"><b>LIVE MATERIAL & MONEY FLOW</b><div>DEPOSIT ↓ EXTRACTION ↓ BATCH ↓ LOCAL COLLECTION ↓ REGIONAL LOGISTICS ↓ REFINERY FEED ↓ REFINED OUTPUT ↓ FACTORY ↓ MARKET ↓ SETTLEMENT ↓ TREASURY</div></section>'+
        '<section class="omega-re2-panel wide"><header>REFINERY / PROCESSING LIVE FLOW <span>THIS TURN</span></header><div class="omega-re2-grid"><div class="omega-re2-panel"><header>INPUT INTO REFINERY</header><div class="omega-re2-scroll">'+Object.keys(d.industry.refiningInput).sort().map(function(k){return'<div class="omega-re2-line"><span>'+esc(k)+'</span><strong>'+money(d.industry.refiningInput[k])+'</strong><em>INPUT</em></div>';}).join('')||'<div class="omega-re2-empty">No refinery input recorded this turn.</div>'+'</div></div><div class="omega-re2-panel"><header>OUTPUT FROM REFINERY / FACTORY</header><div class="omega-re2-scroll">'+Object.keys(d.industry.refiningOutput).sort().map(function(k){return'<div class="omega-re2-line"><span>'+esc(k)+'</span><strong>+'+money(d.industry.refiningOutput[k])+'</strong><em>OUTPUT</em></div>';}).join('')||'<div class="omega-re2-empty">No refinery output recorded this turn.</div>'+'</div></div></div></section>'+'<section class="omega-re2-panel wide"><header>RESOURCE TRANSPORTATION <span>'+d.transport.shipmentCount+' shipments · '+d.transport.inTransit+' in transit · '+d.transport.delivered+' delivered</span></header><div class="omega-re2-scroll">'+d.transport.shipments.slice(-64).map(function(s){return'<div class="omega-re2-row"><b>'+esc(s.resourceId)+'</b><span>'+esc(s.kind||'BATCH')+'</span><strong>'+money(s.quantity)+'</strong><strong>'+money(s.deliveredQuantity)+'</strong><em>'+esc(s.status)+'</em><small>'+esc(s.currentStage)+' → '+esc(s.destinationStage)+' · '+esc(s.networkMode||'UNSPECIFIED_LOGISTICS')+'</small></div>';}).join('')||'<div class="omega-re2-empty">No resource transport shipments are recorded.</div>'+'</div></section>'+'<section class="omega-re2-grid"><div class="omega-re2-panel wide"><header>OPERATING MINE REGISTER <span>'+d.mines.total+' sites</span></header><div class="omega-re2-scroll">'+mines+'</div></div>'+
