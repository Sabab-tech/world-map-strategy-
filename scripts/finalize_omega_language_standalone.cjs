...
const ALL=[...JSON_SOURCES,'offline_lexicon.json',...JS_SOURCES];
...
const snap=f=>{const b=fs.readFileSync(path.join(root,f));const t=decode(b);return{bytes:b,text:t,rawSha256:sha(b),textSha256:sha(Buffer.from(t))}};
...
for(const f of JS_SOURCES){const s=snap(f);archive[f]=s.text;meta[f]={bytes:s.bytes.length,rawSha256:s.rawSha256,textSha256:s.textSha256,archived:true}}
...
console.log(JSON.stringify({ok:true,bundleBytes:Buffer.byteLength(out),version:build.version,lexiconRecovery:build.lexiconRecovery,sourceStringCount:build.sourceStringCount},null,2));
