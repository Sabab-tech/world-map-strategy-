...
assert(/(?:function\s+build\s*|build\s*=\s*function\s*)\(\s*question\s*,/.test(contextBridge), 'context bridge must expose a callable canonical request-body build interface');
...
assert(!resourceBridge.includes('expectedCountryIdCount:197'), 'resource bridge must not hardcode a country-count target');
assert(resourceBridge.includes('canonicalCountryBridge'), 'resource bridge must delegate country identity to the canonical registry');
assert(resourceBridge.includes('waitForCanonicalCountryBridge'), 'resource bridge must wait for canonical country identity readiness');
assert(resourceBridge.includes('countryIdCoverageComplete'), 'resource bridge must report source-derived country coverage status');
