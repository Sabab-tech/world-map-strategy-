/* OMEGA MINISTER BOOTSTRAP v1.3.0 */
(function(global){'use strict';
  const scripts=['/minister_behavior_engine.js','/omega_minister_integration.js','/minister_communication_engine.js'];
  ...
  const boot=()=>{
    ...
    const patchProfile=()=>{
      if(ui.__canonicalMinisterProfilePatchedV3)return;ui.__canonicalMinisterProfilePatchedV3=true;
    ...
    patchProfile();
    Promise.all([loadConfig(),loadMinisters()]).then(()=>Promise.all(scripts.map(loadScript))).then(()=>{patchProfile();registry.consistencyCheck();global.dispatchEvent(new CustomEvent('MINISTER_RUNTIME_READY',{detail:{registryVersion:registry.VERSION,behaviorVersion:global.OmegaMinisterBehavior?.VERSION||null,communicationVersion:global.OmegaMinisterCommunication?.version||null}}));}).catch(e=>console.warn('[Minister Bootstrap] runtime bridge load:',e.message));
    return true;
  };
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
  global.OmegaMinisterBootstrap={version:'1.3.0',boot};
