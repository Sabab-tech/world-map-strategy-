...
(function(global){
  ...
  const W=s=>N(s).split(/\s+/).filter(Boolean), C=s=>W(s).filter(x=>!STOP.has(x));
  function frame(question){
    const raw=String(question||''),n=N(raw),w=new Set(W(raw)),bn=BN.test(raw);
    ...
    if(/\b(potential|prospect|prospects|potentiality|importance|important|valuable|value)\b|সম্ভাবনা|সম্ভাবনাময়|সম্ভাবনাময়|গুরুত্ব|মূল্যবান|মূল্য/.test(n)) operation='RESOURCE_POTENTIAL';
    else if(/\b(import|imports|importing|supplier|suppliers|buy|buying|purchase|purchasing|from which country|source)\b|আমদানি|সরবরাহকারী|কোন দেশ থেকে/.test(n)) operation='IMPORT';
    ...
    const focus=w.has('why')||w.has('because')||/কেন|কারণ/.test(n)?'CAUSE':w.has('when')||/কখন/.test(n)?'TIME':w.has('how')||/কিভাবে|কীভাবে/.test(n)?'METHOD':w.has('which')||/কোন/.test(n)?'SELECTION':operation==='RESOURCE_POTENTIAL'?'POTENTIAL':'FACT';
    ...
    const relation=operation==='IMPORT'?'IMPORT_FROM':operation==='EXPORT'?'EXPORT_TO':operation==='SECURITY'?'SECURITY_OF':operation==='COMPARE'?'COMPARE':operation==='RESOURCE_POTENTIAL'?'POTENTIAL_OF':null;
  ...
  global.OmegaSemanticPlanner=Object.freeze({version:'1.1.0',frame,attachEntities});
})(window);