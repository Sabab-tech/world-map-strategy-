/**
 * OMEGA REASONING DISPATCHER v2.0.0
 * Evidence-bound bridge between semantic routing and the existing 40-stage OS.
 * Stages 21-40 are made distinct here without creating another cognitive engine:
 * each wrapper preserves the original stage, then adds one specialized artifact.
 */
(function(global){'use strict';
  ...
  const STAGE_SPECS={
    21:{name:'EVIDENCE_LEDGER',depends:[],parallelEligible:true},
    22:{name:'PROVENANCE_AUDIT',depends:[21],parallelEligible:false},
    23:{name:'DATA_COMPLETENESS_AUDIT',depends:[21],parallelEligible:true},
    24:{name:'CONSISTENCY_AND_CONFLICT_CHECK',depends:[21],parallelEligible:true},
    25:{name:'BELIEF_REVISION',depends:[21,22,24],parallelEligible:false},
    26:{name:'TEMPORAL_CONTEXT_MODEL',depends:[21,23],parallelEligible:true},
    27:{name:'CAUSAL_LINK_GRAPH',depends:[21,24],parallelEligible:true},
    28:{name:'DEPENDENCY_RECONSTRUCTION',depends:[27],parallelEligible:false},
    29:{name:'RESOURCE_BALANCE_ANALYSIS',depends:[21,23],parallelEligible:true},
    30:{name:'CAPACITY_CONSTRAINT_ANALYSIS',depends:[21,24],parallelEligible:true},
    31:{name:'TRADE_AND_CHOKEPOINT_EXPOSURE',depends:[21,23],parallelEligible:true},
    32:{name:'STRATEGIC_VULNERABILITY_SYNTHESIS',depends:[27,28,29,30,31],parallelEligible:false},
    33:{name:'OPTION_EVIDENCE_ALIGNMENT',depends:[20,29,30,31],parallelEligible:false},
    34:{name:'COUNTERFACTUAL_FRAME',depends:[29,30,31,33],parallelEligible:false},
    35:{name:'MEMORY_CONSOLIDATION',depends:[21,25,26,27,32],parallelEligible:false},
    36:{name:'RED_TEAM_RECHECK',depends:[33,34],parallelEligible:false},
    37:{name:'DECISION_CONFIDENCE_SYNTHESIS',depends:[23,24,32,36],parallelEligible:false},
    38:{name:'ANSWER_EVIDENCE_PACKET',depends:[21,22,23,24,25,27,28,29,30,31,32,33,34,36,37],parallelEligible:false},
    39:{name:'ANSWER_CONTRACT',depends:[37,38],parallelEligible:false},
    40:{name:'FINAL_AI_HANDOFF',depends:[38,39],parallelEligible:false}
  };
  ...
  function telemetry(context){
    ...
    const num=(...keys)=>{for(const k of keys){const n=Number(gs?.[k]);if(Number.isFinite(n))return n;}return undefined;};
  ...
  function store(ctx,id,spec,status,output,error){ensureStores(ctx);const artifact={stage:id,name:spec.name,status,dependsOn:spec.depends||[],parallelEligible:!!spec.parallelEligible,source:'RUNTIME_COGNITIVE_STAGE',output:clone(output)||{}};if(error)artifact.error=text(error.message||error);ctx.cognitiveStageArtifacts[id]=artifact;ctx.stageContracts[id]={stage:id,name:spec.name,status,dependsOn:spec.depends||[],parallelEligible:!!spec.parallelEligible};if(error)ctx.stageErrors.push({stage:id,error:artifact.error});return ctx;}
  function callOriginal(ctx,os,original){try{return original&&typeof original.execute==='function'?original.execute(ctx,os):ctx;}catch(error){ctx.stageErrors ||= [];ctx.stageErrors.push({stage:'ORIGINAL',error:text(error.message)});return ctx;}}
  function latest(ctx,id){return ctx.cognitiveStageArtifacts?.[id]?.output||null;}
  function runSpecialized(id,ctx,os){
    const spec=STAGE_SPECS[id];ensureStores(ctx);let out={};let status='READY';
    try{
      const t=ctx.telemetry||{};const ev=ctx.liveEvidence||{};const evidence=ev.facts||[];
      switch(id){
        case 21:{ctx.evidenceLedger={facts:evidence,unknowns:ev.unknowns||[],telemetry:clone(t),question:ctx.prompt,domain:ctx.domain};out=ctx.evidenceLedger;break;}
        ...
        case 24:{const conflicts=[];for(const [k,v] of Object.entries(t.inventory||{})){const q=n(v?.quantity);if(q!=null&&q<0)conflicts.push({field:`inventory.${k}.quantity`,type:'NEGATIVE_VALUE',value:q});const b=n(v?.bufferDays);if(b!=null&&b<0)conflicts.push({field:`inventory.${k}.bufferDays`,type:'NEGATIVE_VALUE',value:b});}if(n(t.processingUtilization)!=null&&(n(t.processingUtilization)<0||n(t.processingUtilization)>1.5))conflicts.push({field:'processingUtilization',type:'OUT_OF_RANGE',value:t.processingUtilization});ctx.consistencyAudit={conflicts,count:conflicts.length,pass:conflicts.length===0};out=ctx.consistencyAudit;break;}
        ...
        case 26:{const h=Array.isArray(ctx.history)?ctx.history.slice(-24):[];ctx.temporalContext={historyCount:h.length,timeHorizon:ctx.timeHorizon||ctx.timestamp||null,priorTurns:h.map(x=>({role:x.role,content:text(x.content).slice(0,500)})),currentTick:ctx.tick||null};out=ctx.temporalContext;break;}
        ...
        case 28:{const path=Array.isArray(ctx.blastRadius?.propagationPath)?ctx.blastRadius.propagationPath.map(x=>clone(x)) : (ctx.causalLinkGraph?.edges||[]);ctx.dependencyGraph={nodes:[...new Set(path.flatMap(x=>[x.from,x.to]).filter(Boolean))],edges:path,depth:path.length};out=ctx.dependencyGraph;break;}
        case 29:{const ex=n(t.dailyExtraction),im=n(t.dailyImports),ind=n(t.dailyIndustrialDemand),civ=n(t.dailyCivilianDemand);const supply=(ex??0)+(im??0),totalDemand=(ind??0)+(civ??0);ctx.resourceBalance={domesticExtraction:ex,imports:im,supply,demand:totalDemand,surplusOrShortage:supply-totalDemand,importDependency:totalDemand>0?Number(((im??0)/totalDemand).toFixed(3)):null};out=ctx.resourceBalance;break;}
        case 30:{const u=range(t.processingUtilization),cap=n(t.processingCapacityBPD);ctx.capacityConstraint={utilization:u,capacity:cap,bindingNode:t.bindingConstraintNode||null,nearSaturation:u!=null?u>=.90:null,overCapacity:u!=null?u>1:null};out=ctx.capacityConstraint;break;}
        ...
        case 32:{const rb=latest(ctx,29),cc=latest(ctx,30),te=latest(ctx,31),ca=latest(ctx,27);const shortage=Math.max(0,-n(rb?.surplusOrShortage)||0),demand=n(rb?.demand)||0;const shortageRate=demand?shortage/demand:0;const score=Number(Math.min(1,Math.max(0,(shortageRate*.45)+(n(cc?.utilization)||0)*.25+(n(te?.exposureScore)||0)*.30)).toFixed(3));ctx.strategicVulnerability={score,drivers:{shortageRate,processingUtilization:cc?.utilization,chokepointExposure:te?.exposureScore,causalEdges:ca?.edges?.length||0}};out=ctx.strategicVulnerability;break;}
        case 33:{const branches=Array.isArray(ctx.searchTreeResults)?ctx.searchTreeResults:[];const evidenceScore=ctx.dataCompleteness?.coverage||0;ctx.optionEvidenceAlignment=branches.map(b=>({optionId:b.optionId,utility:n(b.expectedUtility),resilience:n(b.resilienceDelta),evidenceCoverage:evidenceScore,alignment:Number(Math.max(0,Math.min(1,(n(b.expectedUtility)||0)*.6+evidenceScore*.4)).toFixed(3))}));out={count:ctx.optionEvidenceAlignment.length,options:ctx.optionEvidenceAlignment};break;}
        ...
        case 35:{const dm=ctx.selfModel?.activeBeliefs;ctx.memoryConsolidation={beliefCount:dm?.size||0,episodeCount:Array.isArray(ctx.deepMemoryEpisodes)?ctx.deepMemoryEpisodes.length:null,trackedIssues:ctx.cognitiveStageArtifacts?.[21]?.output?.issues||null,consolidationReady:true};out=ctx.memoryConsolidation;break;}
        case 36:{let audit=null;try{const C=global.DeepRedTeamCritic;if(C&&typeof C.auditDecision==='function'&&ctx.selectedBranch)audit=C.auditDecision(ctx.selectedBranch,ctx.searchTreeResults||[],ctx);}catch(_){ }ctx.redTeamRecheck={available:!!audit,passed:audit?.passed??null,worstCase:audit?.worstCaseScenario||null,mandatoryContingency:audit?.mandatoryContingency||null};out=ctx.redTeamRecheck;break;}
        case 37:{const completeness=range(ctx.dataCompleteness?.coverage)||0;const consistency=ctx.consistencyAudit?.pass===false?0:1;const uncertainty=n(ctx.uncertaintyIndex);const redTeam=ctx.redTeamRecheck?.passed===false?.25:1;const confidence=Math.max(0,Math.min(1,(completeness*.30)+(consistency*.25)+((1-(uncertainty??0))*.25)+(redTeam*.20)));ctx.decisionConfidence={score:Number(confidence.toFixed(3)),evidenceCoverage:completeness,consistency:!!ctx.consistencyAudit?.pass,uncertaintyIndex:uncertainty,redTeam:ctx.redTeamRecheck?.passed??null};out=ctx.decisionConfidence;break;}
        case 38:{ctx.answerEvidencePacket={question:ctx.prompt,language:ctx.language||null,operation:ctx.intent||null,rootCause:ctx.rootCause||null,facts:ctx.evidenceLedger?.facts||[],unknowns:ctx.evidenceLedger?.unknowns||[],calculations:{resourceBalance:latest(ctx,29),capacity:latest(ctx,30),tradeExposure:latest(ctx,31),strategicVulnerability:latest(ctx,32)},decision:clone(ctx.finalDecision||null),confidence:latest(ctx,37),provenance:latest(ctx,22)};out=ctx.answerEvidencePacket;break;}
        case 39:{ctx.answerContract={evidenceOnly: true,separateFactInferenceUnknown:true,doNotInventMissingValues:true,sourcePacket:'STAGE_38',requiredGroundingFields:['facts','unknowns','calculations','decision','confidence'],answerStyle:'NATURAL_LANGUAGE_SYNTHESIS'};out=ctx.answerContract;break;}
        case 40:{ctx.aiGroundingPacket={version:VERSION,stageCount:40,stageContracts:clone(ctx.stageContracts),stageArtifacts:clone(ctx.cognitiveStageArtifacts),answerEvidence:clone(ctx.answerEvidencePacket),answerContract:clone(ctx.answerContract),retrievalOrder:[38,39],groundingBoundary:'RUNTIME_STAGE_ARTIFACTS_ONLY'};out=ctx.aiGroundingPacket;break;}
  ...
  function execute40(question,category,evidence,context){
    ...
    if(!engine)return{available:false,reason:'COGNITIVE_ENGINE_UNAVAILABLE',stagesRequested:40,stagesExecuted:0,full40:false};
    ...
    const originals=stages.slice(20,40),first=stages[0];
    const live=telemetry(context);
    const firstWrapper={id:first.id,name:first.name,execute(ctx,os){ctx.countryId=context?.countryId||context?.countryCode||ctx.countryId;ctx.targetCountry=context?.countryName||ctx.targetCountry;ctx.language=context?.language||ctx.language||'en';ctx.domain=text(context?.ministryId||category||ctx.domain).toUpperCase();ctx.telemetry=clone(live);ctx.liveEvidence=clone(evidence);ctx.history=clone(context?.history||[]);ctx.timeHorizon=context?.timeHorizon||'CURRENT';ctx.evidenceBoundary='RUNTIME_DETERMINISTIC_RESULT_ONLY';ctx.prohibitWorldStateInvention=true;ensureStores(ctx);return first.execute(ctx,os);}};
    stages[0]=firstWrapper;
    try{
      for(let i=0;i<20;i++){const id=21+i,spec=STAGE_SPECS[id],original=originals[i];stages[20+i]={id,name:spec.name,execute(ctx,os){callOriginal(ctx,os,original);return runSpecialized(id,ctx,os);}}}
  ...
  function evidenceSummaryForAnswer(result){return result?.aiGroundingPacket||result?.answerEvidencePacket||null;}
  function dispatch(question,parsed,result,common={}){
    ...
    const category=categoryOf(operation),cognitive=execute40(question,category,evidence,common);
