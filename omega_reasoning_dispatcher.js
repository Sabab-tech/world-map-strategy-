/**
 * OMEGA REASONING DISPATCHER v1.0.0
...
(function(global){'use strict';
  ...
  function cognitiveCategory(operation){
    const map = {
      ...
      CALCULATE:'DETERMINISTIC_CALCULATION', QUANTITY:'QUANTITATIVE_ANALYSIS',
  ...
  function safeCognitiveCall(question, category, evidence, context){
    ...
    try{
      const country = context?.countryObject || context?.country || context?.countryName || context?.countryId || null;
      const target = {
  ...
  function extractText(raw, language){
    if(typeof raw === 'string' && raw.trim()) return raw.trim();
    if(!obj(raw)) return '';
    const keys = language === 'bn'
      ? ['answer','text','summary','recommendation','conclusion','analysis','briefing']
      : ['answer','text','summary','recommendation','conclusion','analysis','briefing'];
    for(const k of keys) if(typeof raw[k] === 'string' && raw[k].trim()) return raw[k].trim();
    return '';
  }
  ...
  function dispatch(question, parsed, result, common={}){
    ...
    const textOut = extractText(cognitive.result, language);
    return {
      version: VERSION,
      operation,
      contract,
  ...
  global.OmegaReasoningDispatcher = Object.freeze({
    VERSION,
    dispatch,
    operations: Object.freeze([...DEEP_OPS])
  });
