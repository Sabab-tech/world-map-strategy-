...
(function(g){
  ...
  function canonicalCountry(value){
    ...
    try{const hit=bridge?.resolveCountry?.(value);if(hit?.id){const raw=clone(hit.raw||null);const source=(raw&&typeof raw==='object'&&raw.raw&&typeof raw.raw==='object')?raw.raw:raw;const canonicalRaw=source?.iso2||source?.countryCode||source?.code||source?.countryId||source?.canonicalId||source?.id||null;return{id:id(canonicalRaw||hit.id),raw,authority:'OMEGA_CANONICAL_COUNTRY_IDENTITY'};}}catch(_){}
