...
(function (g) {
  ...
  function countResources() {
    try {
      return Number(resourceBridge?.diagnostics?.()?.resourceTypeCount || 0);
    } catch (_) {
      return 0;
    }
  }
  ...
  function diagnostics() {
    let base = {};
    try {
      base = originalDiagnostics ? (originalDiagnostics() || {}) : {};
    } catch (error) {
    ...
    return queryReady
      ? { ...base, status: 'READY', repositoryScan: { discovered, loaded, failed }, identityIndex: { countries, resources }, queryReady: true, reason: null }
      : { status: 'BROKEN', repositoryScan: { discovered, loaded, failed }, identityIndex: { countries, resources }, queryReady: false, reason: 'DATA_INDEX_EMPTY' };
  }

  function unavailable() {
    const d = diagnostics();
    return { ok: false, status: 'DATA_UNAVAILABLE', value: null, evidence: [], trace: [{ step: 'DATA_AVAILABILITY_GATE', status: 'BLOCKED', reason: d.reason }], dataAccess: d, reason: d.reason };
  }
  ...
  if (offline && !offline.__omegaFailClosedGuardV101) {
    offline.execute = function (...args) {
      ...
      try { return originalOfflineExecute(...args) ?? unavailable(); }
    ...
    offline.executeAsync = async function (...args) {
      ...
      try { return (await originalOfflineExecuteAsync(...args)) ?? unavailable(); }
      catch (_) { return unavailable(); }
    };
    ...
    if (production && originalProductionExecute && !production.__omegaFailClosedProductionV101) {
      production.execute = function (...args) {
        if (!ready()) return unavailable();
        try { return originalProductionExecute(...args) ?? unavailable(); }
        catch (_) { return unavailable(); }
      };
