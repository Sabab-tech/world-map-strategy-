...
(function (g) {
  ...
  const originalOfflineExecute = offline && typeof offline.execute === 'function'
    ? offline.execute.bind(offline)
    : null;
  const originalOfflineExecuteAsync = offline && typeof offline.executeAsync === 'function'
    ? offline.executeAsync.bind(offline)
    : null;
  const originalDiagnostics = offline && typeof offline.diagnostics === 'function'
    ? offline.diagnostics.bind(offline)
    : null;
  const originalDiagnosticsAsync = offline && typeof offline.diagnosticsAsync === 'function'
    ? offline.diagnosticsAsync.bind(offline)
    : null;
  const originalProductionExecute = production && typeof production.execute === 'function'
    ? production.execute.bind(production)
    : null;
  ...
  function baseDiagnostics() {
    ...
    try {
      ...
      return { diagnosticsError: error?.message || String(error) };
    }
  }
  ...
  function resourceCount() {
    try {
      ...
      return Number(d.resourceTypeCount || 0);
    } catch (_) {
      return 0;
    }
  }
  ...
  function ready() {
    return diagnostics().queryReady === true;
  }

  if (offline && !offline.__omegaFailClosedGuardV101) {
    offline.execute = function (...args) {
      if (!ready()) return unavailable();
      if (!originalOfflineExecute) return unavailable({ reason: 'DATA_UNAVAILABLE' });
      try {
        const result = originalOfflineExecute(...args);
        return result == null ? unavailable({ reason: 'DATA_UNAVAILABLE' }) : result;
      } catch (error) {
    ...
    offline.executeAsync = async function (...args) {
      if (!ready()) return unavailable();
      if (!originalOfflineExecuteAsync) return unavailable({ reason: 'DATA_UNAVAILABLE' });
      try {
    ...
    if (production && originalProductionExecute && !production.__omegaFailClosedProductionV101) {
      ...
      Object.defineProperty(production, '__omegaFailClosedProductionV101', {
        value: true,
        enumerable: false,
        configurable: false
      });
    ...
    Object.defineProperty(offline, '__omegaFailClosedGuardV101', {
      value: true,
      enumerable: false,
      configurable: false
    });
