...
(function(g){
  ...
  function occurrenceRows(c){
    ...
    try{
      ...
      for(const occ of occurrences){
        ...
        let resourceId=rid(occ.resourceTypeId||occ.resourceTypeKey);
        ...
        if(/\bcoal\b|coal basin|bituminous|anthracite/.test(semanticName)&&resourceId==='iron_ore'){
          ...
          if(repaired)resourceId=repaired;
        }
  ...
  function extractHandler(cmd,ctx){
    ...
    const inventoryLedger=Array.isArray(ctx.stateTransaction.get('resource.inventoryLedger'))?clone(ctx.stateTransaction.get('resource.inventoryLedger')):[];
    for(const x of selected){
      const reserve=r.getReserveState(x.occurrenceKey);
      if(!reserve||reserve.residualQuantity<=0){
        const row={occurrenceKey:x.occurrenceKey,resourceId:x.resourceId,status:'BLOCKED',reason:reserve?'RESERVE_EXHAUSTED':'RESERVE_STATE_UNAVAILABLE',simulationTurn:turn()};
        blocked.push(row);mineOutputs[x.occurrenceKey]=row;continue;
      }
      if(!['ACTIVE_EXTRACTION','DEPLETING','RESERVE_DEPLETING'].some(s=>String(reserve.operationalStatus||'').toUpperCase().includes(s))){
        const row={occurrenceKey:x.occurrenceKey,resourceId:x.resourceId,status:'BLOCKED',reason:'MINE_NOT_OPERATIONAL',operationalStatus:reserve.operationalStatus||'UNKNOWN',simulationTurn:turn()};
        blocked.push(row);mineOutputs[x.occurrenceKey]=row;continue;
      }
      let capacity=null;try{capacity=r.getCapacityForOccurrence(x.occurrenceKey);}catch(_){}
      if(!capacity){
        const row={occurrenceKey:x.occurrenceKey,resourceId:x.resourceId,status:'BLOCKED',reason:'EXTRACTION_CAPACITY_UNAVAILABLE',simulationTurn:turn()};
        blocked.push(row);mineOutputs[x.occurrenceKey]=row;continue;
      }
      let windowQuantity=0;let capacityTrace=null;
      try{capacityTrace=capacity.computeWindowCapacity(DAY_HOURS);windowQuantity=n(capacityTrace?.windowCapacity)||0;}catch(_){windowQuantity=n(capacity.nominalRate)||0;}
      if(windowQuantity<=0){
        const row={occurrenceKey:x.occurrenceKey,resourceId:x.resourceId,status:'BLOCKED',reason:'EXTRACTION_WINDOW_ZERO',capacityTrace:clone(capacityTrace),simulationTurn:turn()};
        blocked.push(row);mineOutputs[x.occurrenceKey]=row;continue;
      }
      ...
      const fallbackBatch={
        batchId:'EXT-BATCH-'+turn()+'-'+c+'-'+String(x.occurrenceKey).replace(/[^A-Z0-9:_-]/gi,''),
        resourceId:resource,materialIdentity:resource,quantity:q,remainingQuantity:q,
        stage:'RAW',ownerCountryCode:c,ownerCompanyId:x.operatorKey||x.ownerKey||'UNOBSERVED',
        sourceBatchIds:[],occurrenceKey:x.occurrenceKey,locationNodeId:'MINE:'+x.occurrenceKey,
        extractionReference:null,processId:null,timestampTurn:turn(),
        provenance:{source:'OMEGA_RESOURCE_ENDOWMENT_RUNTIME',extractionId:'EXT-'+turn()+'-'+c+'-'+String(x.occurrenceKey).replace(/[^A-Z0-9:_-]/gi,''),depositKey:x.depositKey,turn:turn()}
      };
      const producedBatch=clone(result.producedBatch?.toJSON?.()||result.producedBatch||fallbackBatch);
      if(!producedBatch.batchId)producedBatch.batchId=fallbackBatch.batchId;
      producedBatch.resourceId=resource;producedBatch.materialIdentity=resource;
      producedBatch.quantity=n(producedBatch.quantity)??q;producedBatch.remainingQuantity=n(producedBatch.remainingQuantity)??producedBatch.quantity;
      producedBatch.ownerCountryCode=producedBatch.ownerCountryCode||c;
      producedBatch.ownerCompanyId=producedBatch.ownerCompanyId||fallbackBatch.ownerCompanyId;
      producedBatch.occurrenceKey=x.occurrenceKey;producedBatch.locationNodeId=producedBatch.locationNodeId||('MINE:'+x.occurrenceKey);
      if(!batches.some(b=>String(b?.batchId)===String(producedBatch.batchId)))batches.push(producedBatch);
      inventoryLedger.push({type:'EXTRACTION_BATCH_CREATED',batchId:producedBatch.batchId,resourceId:resource,quantity:producedBatch.quantity,occurrenceKey:x.occurrenceKey,turn:turn(),extractionId:'EXT-'+turn()+'-'+c+'-'+String(x.occurrenceKey).replace(/[^A-Z0-9:_-]/gi,'')});
      mineOutputs[x.occurrenceKey]={
        ...
        batchId:producedBatch.batchId,locationNodeId:producedBatch.locationNodeId
      };
      ...
      const record={
        ...
        producedBatch:clone(producedBatch),
  ...
  function transportReserveCargoHandler(cmd,ctx){
    const p=cmd?.payload||{},rid=String(p.resourceId||'').trim(),q=n(p.quantity),sourceNodeId=String(p.sourceNodeId||'').trim(),allowAny=p.allowAnySource===true;
    if(!rid||q===null||q<=0)return{accepted:false,reason:'TRANSPORT_RESERVE_INPUT_INVALID'};
    const inv=clone(ctx.stateTransaction.get('resource.inventory')||{}),key=Object.prototype.hasOwnProperty.call(inv,rid)?rid:Object.keys(inv).find(k=>String(k).toLowerCase()===rid.toLowerCase());
    const stock=key===undefined?null:n(inv[key]);
    if(stock===null||stock<q)return{accepted:false,reason:'TRANSPORT_SOURCE_INVENTORY_INSUFFICIENT',resourceId:rid,requested:q,available:stock===null?0:stock};
    const batches=Array.isArray(ctx.stateTransaction.get('resource.batches'))?clone(ctx.stateTransaction.get('resource.batches')):[],allocations=[];let rem=q;
    for(const b of batches){
      if(rem<=1e-9)break;
      if(String(b?.resourceId||b?.materialIdentity||'').toLowerCase()!==rid.toLowerCase())continue;
      const loc=String(b?.locationNodeId||'STOCKPILE:'+canonical(ctx.countryId));
      if(!allowAny&&sourceNodeId&&loc!==sourceNodeId)continue;
      const free=n(b?.remainingQuantity!=null?b.remainingQuantity:b?.quantity)||0;if(free<=0)continue;
      const take=Math.min(free,rem);b.remainingQuantity=free-take;b.inTransitQuantity=(n(b.inTransitQuantity)||0)+take;rem-=take;
      allocations.push({batchId:b.batchId,quantity:take,sourceNodeId:loc,ownerCompanyId:b.ownerCompanyId||'UNKNOWN_SOURCE',stage:b.stage||'RAW'});
    }
    if(rem>1e-9)return{accepted:false,reason:'TRANSPORT_SOURCE_BATCH_LINEAGE_INSUFFICIENT',resourceId:rid,requested:q,unallocated:rem};
    inv[key]=stock-q;
    ...
    ledger.push({type:'TRANSPORT_CARGO_RESERVED',resourceId:rid,quantity:q,sourceNodeId:sourceNodeId||'ANY',allowAnySource:allowAny,batchAllocations:clone(allocations),turn:turn(),correlationId:p.correlationId||null});
    while(ledger.length>2048)ledger.shift();
    ctx.stateTransaction.set('resource.inventory',inv);ctx.stateTransaction.set('resource.batches',batches);ctx.stateTransaction.set('resource.inventoryLedger',ledger);
    return{accepted:true,resourceId:rid,quantity:q,allocations:allocations,inventoryAfter:inv[key]};
  }

  function transportCancelCargoHandler(cmd,ctx){
    const p=cmd?.payload||{},rid=String(p.resourceId||'').trim(),q=n(p.quantity),allocations=Array.isArray(p.allocations)?p.allocations:[];
    if(!rid||q===null||q<=0)return{accepted:false,reason:'TRANSPORT_CANCEL_INPUT_INVALID'};
    const inv=clone(ctx.stateTransaction.get('resource.inventory')||{}),key=Object.prototype.hasOwnProperty.call(inv,rid)?rid:Object.keys(inv).find(k=>String(k).toLowerCase()===rid.toLowerCase());
    if(key===undefined)return{accepted:false,reason:'RESOURCE_NOT_OBSERVED'};
    inv[key]=(n(inv[key])||0)+q;
    const batches=Array.isArray(ctx.stateTransaction.get('resource.batches'))?clone(ctx.stateTransaction.get('resource.batches')):[],byId=new Map(batches.map(b=>[String(b?.batchId),b]));
    for(const a of allocations){const b=byId.get(String(a?.batchId));if(!b)continue;b.remainingQuantity=(n(b.remainingQuantity)||0)+(n(a.quantity)||0);b.inTransitQuantity=Math.max(0,(n(b.inTransitQuantity)||0)-(n(a.quantity)||0));}
    ctx.stateTransaction.set('resource.inventory',inv);ctx.stateTransaction.set('resource.batches',batches);
  ...
  function transportCreditDeliveryHandler(cmd,ctx){
    ...
    if(facilityId){
      ...
      facilityInventory[facilityId][rid]=(n(facilityInventory[facilityId][rid])||0)+q;
      ctx.stateTransaction.set('resource.facilityInventory',facilityInventory);
    }
    const ledger=Array.isArray(ctx.stateTransaction.get('resource.inventoryLedger'))?clone(ctx.stateTransaction.get('resource.inventoryLedger')):[];
    ledger.push({type:'TRANSPORT_DELIVERY_CREDIT',shipmentId,resourceId:rid,quantity:q,batchId,sourceBatchIds:Array.isArray(p.sourceBatchIds)?p.sourceBatchIds.slice():[],destinationNodeId,facilityId,turn:turn()});
    while(ledger.length>2048)ledger.shift();
    ctx.stateTransaction.set('resource.inventory',inv);ctx.stateTransaction.set('resource.batches',batches);ctx.stateTransaction.set('resource.inventoryLedger',ledger);
    return{accepted:true,resourceId:rid,quantity:q,batchId};
  }
