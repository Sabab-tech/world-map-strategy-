/* OMEGA MINISTER QUERY ROUTER v12.0.0-DEEP-CORE-AUTHORITY
 * Deep Core is the authoritative data retrieval path. Production/AI layers may explain
 * verified evidence, but may not replace or invent authoritative data.
 */
(function(global){'use strict';
const VERSION='12.0.0-DEEP-CORE-AUTHORITY';
...
let universalPromise=null;const loadedScripts=new Set();

function parse(q,ctx={}){
  try{if(brain()?.parse){const p=brain().parse(q,ctx);if(p&&p.contractVersion)return p}}catch(_){}
...
function route(q,identity={},world={}){const ctx={...identity,...world,history:world.history||identity.history||[]},p=parse(q,ctx),result=executeSemantic(p,ctx);return{version:VERSION,intent:p.operation||p.question?.type||'UNKNOWN',domain:p.intent||p.targetDomain||'GENERAL',entities:Object.values(p.entities||{}).filter(e=>e?.id).map(e=>({type:e.type||null,id:e.id,confidence:e.confidence||null,source:e.source||null,surface:e.surface||null})),requiredData:Object.values(p.entities||{}).filter(e=>e?.id).map(e=>e.type||null).filter(Boolean),semantic:p,result,dataFound:result?.status==='VERIFIED_FACT',executable:p.executable!==false&&result?.status!=='NO_AUTHORITATIVE_EXECUTOR'} }
...
function enqueue(q){if(universalPromise===null)universalPromise=ensureCanonicalBrowserStack();return universalPromise.then(u=>{if(u?.enqueue)return u.enqueue(q);return route(q).result}).catch(e=>({ok:false,status:'CANONICAL_BROWSER_ENTRY_FAILED',error:e.message,value:null,evidence:[]}))}
function interceptSubmission(event){const button=event?.target?.closest?.('#btn-submit-interrogation');if(!button)return false;const{input,q}=getQuestionFromUI();if(!q)return false;event.preventDefault();event.stopImmediatePropagation();if(input)input.value='';enqueue(q).catch(()=>{});return true}
function interceptEnter(event){if(event.key!=='Enter'||event.shiftKey||event.isComposing||event.target?.id!=='interrogation-input')return false;const q=S(event.target.value);if(!q)return false;event.preventDefault();event.stopImmediatePropagation();event.target.value='';enqueue(q).catch(()=>{});return true}
function installEntryInterceptors(){if(typeof document==='undefined'||document.__OMEGA_CANONICAL_ENTRY_V12)return false;document.__OMEGA_CANONICAL_ENTRY_V12=true;document.addEventListener('click',interceptSubmission,true);document.addEventListener('keydown',interceptEnter,true);return true}
function canonicalMinisterAnswer(questionText,minister,countryKey,countryDetails){const i=integrity();if(!i?.buildAnswerPlan)return null;const gs=global.Game?.state||global.gameState||global.Omega?.World?.state||{};const identity={countryId:S(countryKey||gs.countryCode||gs.countryId||''),countryName:S(countryDetails?.name||countryDetails?.countryName||gs.countryName||''),ministerId:S(minister?.id||minister?.ministerId||''),ministerName:S(minister?.name||minister?.ministerName||minister?.displayName||''),ministerRole:S(minister?.role||minister?.title||''),ministryId:S(minister?.ministryId||''),gameState:gs};try{const plan=i.buildAnswerPlan(S(questionText),identity,gs,[]);if(!plan)return null;if(plan?.result?.status!=='VERIFIED_FACT')return null;const text=typeof i.formatOfflineAnswer==='function'?i.formatOfflineAnswer(plan):'';return{text:String(text||''),impact:plan?.semantic?.identityAuthority||'DEEP_CORE',aiPowered:false,canonical:true,plan}}catch(_){return null}}
...
const API=Object.freeze({VERSION,parse,executeSemantic,routeMinisterQuery:route,resolveKnowledgeQuery,detectIntent,learn,recall,offlineQuery:resolveKnowledgeQuery,ensureCanonicalBrowserStack,enqueue});
global.MinisterQueryRouter=API;installEntryInterceptors();installCognitiveFacade();if(typeof module!=='undefined'&&module.exports)module.exports=API;
